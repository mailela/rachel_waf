...
class ConfigManager
{
	...
	public $waf_manager_white_ip_die=0;//非法进入管理页面时是否直接退出
	...
	public $head_string = "HTTP/1.1 555 RACHEL WAF DENY";  // 被拦截时输出头信息
	...
	public $allow_ddos_time = 200;  // 每秒最多5个访问 
...
class Tools
{
	...
	public function enter()
	{
		...
		return  $php_file . "?" . $config->WAF_FLAG_ENTER;
	}
...
class WAF
{
	...
	function watch_ddos()
	{
		...
		$IP = str_replace(":", '_', $IP);
		$date = date('H_i_s');
		$IP_dir = Tools::getInstance()->fix_path($this->ip_dir . $this->dispersed($IP) . "/");
		$this->e_mkdir($IP_dir);
	...
	{
		global $config;
		$ip = Tools::getInstance()->getRealIp();
		if (!_Env::get("WAF.WAF_BLACK_IP", $config->waf_black_ip)) {
			return;
		}
		$ip_range = _Env::get("WAF.BLACK_IP", $config->ip_address_deny);
		$is_allow = Tools::getInstance()->in_Ip_address($ip, $ip_range, $allow_range);  // 监测headers
		if (empty($ip_range)) {
			return;
		}
		if ($is_allow) {
	...
	// 允许管理的的IP段
	function waf_manage_white_ip($is_die = true)
	{

		global $config;
		$ip = Tools::getInstance()->getRealIp();
		$ip_range = _Env::get("WAF.ALLOW_MANAGE_IP", "");
		$is_allow = Tools::getInstance()->in_Ip_address($ip, $ip_range, $allow_range);
		$is_allow_cfg = Tools::getInstance()->in_Ip_address($ip, $config->ip_address_manage_allow, $allow_range);
...
class ui
{
	...
	function __construct($resource_path = "")
	{
		global $config;
		if (!defined('STDIN')) {
			WAF::getInstance()->waf_manage_white_ip($config->waf_manager_white_ip_die);
		}
	...
	function lax($html, $content = "")
	{
		...
		$php_file = substr(str_replace("//", "/", @$_SERVER['DOCUMENT_URI']), 1);
		$html = preg_replace("/waf_static\//", Tools::getInstance()->enter()  . "=resource&resource=/waf_static/", $html);
...
class Api
{
	...
	function __construct()
	{
		global $config;
		// 使用示例
		$this->token_store = new FileCache(__WAF_ROOT__ . "/sessions/" . Tools::getInstance()->host(), $config->max_token_live_time); // 缓存目录和过期时间（秒）
	}
	...
	{
		return Tools::getInstance()->domain() . Tools::getInstance()->enter();
	}
...
class Langs
{
	...
	function __construct()
	{
		$this->group = array(
			...
			"manage" => array(
				"WAF_FLAG_ENTER" => array("title" => "管理入口", "desc" => "如：配置为WAF,管理地址为:(" . Tools::getInstance()->domain().Tools::getInstance()->enter() . "=ui)"),
				"waf_manager_white_ip_die" => array("title" => "拦截", "desc" => "拦截非法进入管理界面操作"),
			...
			"ddos" => array(
				"allow_ddos_time" => array("title" => "频次", "desc" => "每秒访问数量(此值不建议过小)"),
...
class Services
{
	...
	function start($argv)
	{
		...
		switch ($this->GET($config->WAF_FLAG_ENTER)) {
			case "api":
				@ob_end_clean();
				$api = new Api();
				$api->call();
			...
			case "ui":
				...
				break;
			case 'resource':
				if (!WAF::getInstance()->waf_manage_white_ip(false)) {
					break;
				}
				ob_end_clean();
				$ui = new ui();
